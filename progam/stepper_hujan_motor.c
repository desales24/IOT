...
const int buzzer = 7;
const int motor_dc = 6;  // Pin untuk motor DC (kipas)
const int DIR_PIN  = 4;  // Pin untuk arah motor stepper
const int STEP_PIN = 5;  // Pin untuk langkah motor stepper
...
void setup() {
  ...
  pinMode(motor_dc, OUTPUT); // Pin motor DC sebagai output
...
void loop() {
  ...
  if (hujan == 1 && !sudahMasuk && !hujanSebelumnya) {
    ...
    // Aktifkan motor DC sebagai kipas saat hujan
    digitalWrite(motor_dc, HIGH);  // Kipas hidup
  }

  // Jika hujan berhenti, keluarkan jemuran
  if (hujan == 0 && sudahMasuk && hujanSebelumnya) {
    // Matikan motor DC setelah hujan berhenti
