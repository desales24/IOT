...
void loop() {
  ...
  if (hujan == 1 && !sudahMasuk && !hujanSebelumnya) {
    ...
    while (millis() - startTime < durasi) {
      unsigned long waktuBerjalan = millis() - startTime;
      ...
      // Nyalakan motor dan LED hujan setelah 5 detik
      if (!motorHidup && waktuBerjalan >= 5000) {
        digitalWrite(motor_dc, HIGH);
        digitalWrite(LED_hujan, HIGH);
        motorHidup = true;
      }

      // Buzzer bunyi putus-putus tiap 200 ms
  ...
  if (hujan == 0 && sudahMasuk && hujanSebelumnya) {
    digitalWrite(motor_dc, LOW);     // Matikan motor
    digitalWrite(LED_hujan, LOW);    // Matikan LED hujan
